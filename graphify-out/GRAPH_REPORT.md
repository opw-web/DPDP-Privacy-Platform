...
- `BoardBreachReport` --references--> `PublicBreach`  [EXTRACTED]
  dpdp-platform/backend/src/modules/breaches/breach.service.ts → dpdp-platform/backend/src/modules/breaches/breach.types.ts
- `PrincipalEvidenceFile` --references--> `PublicInformationRequest`  [EXTRACTED]
  dpdp-platform/backend/src/modules/evidence/principal-evidence.service.ts → dpdp-platform/backend/src/modules/evidence/non-disclosure.ts
...
## Knowledge Gaps
- **1199 isolated node(s):** `NotificationRow`, `PublicNotification`, `PrincipalNoticeSummary`, `PrincipalNoticeView`, `AppointingAuthority` (+1194 more)
...
- **What connects `NotificationRow`, `PublicNotification`, `PrincipalNoticeSummary` to the rest of the system?**

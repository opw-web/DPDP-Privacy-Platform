...
- `supportRoutes()` --indirect_call--> `onlyGet()`  [INFERRED]
  demo-company-server/src/routes/support.ts → demo-company-server/src/methodGate.ts
...
## Knowledge Gaps
- **404 isolated node(s):** `name`, `version`, `private`, `description`, `main` (+399 more)
...
- **Why does `AuditService` connect `PrismaService` to `employees.service.ts`, `UpdateOrganizationDto`, `CreateSharingActivityDto`, `registers.module.ts`, `prisma.service.ts`, `retention.service.ts`, `principal-auth.service.ts`, `data-sources.service.ts`, `employees.module.ts`, `mappings.service.ts`, `purposes.service.ts`, `recipients.service.ts`, `.record`, `audit.service.ts`?**

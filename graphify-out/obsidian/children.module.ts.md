---
source_file: "dpdp-platform/backend/src/modules/children/children.module.ts"
type: "code"
community: "app.module.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/appmodulets
---

# children.module.ts

## Connections
- [[AgeStatusController]] - `imports` [EXTRACTED]
- [[AgeStatusService]] - `imports` [EXTRACTED]
- [[AuditModule]] - `imports` [EXTRACTED]
- [[ChildExemptionsController]] - `imports` [EXTRACTED]
- [[ChildExemptionsService]] - `imports` [EXTRACTED]
- [[ChildrenModule]] - `contains` [EXTRACTED]
- [[GuardiansController]] - `imports` [EXTRACTED]
- [[GuardiansService]] - `imports` [EXTRACTED]
- [[MaskingModule]] - `imports` [EXTRACTED]
- [[age-status.controller.ts]] - `imports_from` [EXTRACTED]
- [[age-status.service.ts]] - `imports_from` [EXTRACTED]
- [[app.module.ts]] - `imports_from` [EXTRACTED]
- [[audit.module.ts]] - `imports_from` [EXTRACTED]
- [[child-exemptions.controller.ts]] - `imports_from` [EXTRACTED]
- [[child-exemptions.service.ts]] - `imports_from` [EXTRACTED]
- [[consents.module.ts]] - `imports_from` [EXTRACTED]
- [[guardians.controller.ts]] - `imports_from` [EXTRACTED]
- [[guardians.service.ts]] - `imports_from` [EXTRACTED]
- [[masking.module.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/appmodulets
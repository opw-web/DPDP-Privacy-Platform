---
source_file: "dpdp-platform/backend/src/modules/principals/principals.module.ts"
type: "code"
community: "app.module.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/appmodulets
---

# principals.module.ts

## Connections
- [[AuditModule]] - `imports` [EXTRACTED]
- [[LineageService]] - `imports` [EXTRACTED]
- [[MaskingModule]] - `imports` [EXTRACTED]
- [[PrincipalRecipientsService]] - `imports` [EXTRACTED]
- [[PrincipalsController]] - `imports` [EXTRACTED]
- [[PrincipalsModule]] - `contains` [EXTRACTED]
- [[PrincipalsService]] - `imports` [EXTRACTED]
- [[app.module.ts]] - `imports_from` [EXTRACTED]
- [[audit.module.ts]] - `imports_from` [EXTRACTED]
- [[evidence.module.ts]] - `imports_from` [EXTRACTED]
- [[lineage.service.ts]] - `imports_from` [EXTRACTED]
- [[masking.module.ts]] - `imports_from` [EXTRACTED]
- [[principal-portal.module.ts]] - `imports_from` [EXTRACTED]
- [[principal-recipients.service.ts]] - `imports_from` [EXTRACTED]
- [[principals.controller.ts]] - `imports_from` [EXTRACTED]
- [[principals.service.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/appmodulets
---
type: community
members: 6
---

# PermissionsController

**Members:** 6 nodes

## Members
- [[dot-constructor()_54]] - code - dpdp-platform/backend/src/modules/employees/permissions.controller.ts
- [[dot-list()_12]] - code - dpdp-platform/backend/src/modules/employees/permissions.controller.ts
- [[ApiTags_11]] - code
- [[Controller_11]] - code
- [[Get_14]] - code
- [[PermissionsController]] - code - dpdp-platform/backend/src/modules/employees/permissions.controller.ts

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/PermissionsController
SORT file.name ASC
```

## Connections to other communities
- 2 edges to [[_COMMUNITY_RequirePermission]]
- 1 edge to [[_COMMUNITY_@nestjscommon]]

## Top bridge nodes
- [[PermissionsController]] - degree 6, connects to 2 communities
- [[dot-list()_12]] - degree 3, connects to 1 community
---
source_file: "dpdp-platform/frontend/src/fiduciary/pages/ChildrenPage.tsx"
type: "code"
community: "api-client.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/api-clientts
---

# ChildrenPage.tsx

## Connections
- [[@tanstackreact-query_1]] - `imports_from` [EXTRACTED]
- [[AgeStatus]] - `contains` [EXTRACTED]
- [[ApiError]] - `imports` [EXTRACTED]
- [[Badge()]] - `imports` [EXTRACTED]
- [[Button]] - `imports` [EXTRACTED]
- [[CHILD_AGE_STATUSES]] - `contains` [EXTRACTED]
- [[Card]] - `imports` [EXTRACTED]
- [[CardContent]] - `imports` [EXTRACTED]
- [[CardDescription]] - `imports` [EXTRACTED]
- [[CardHeader]] - `imports` [EXTRACTED]
- [[CardTitle]] - `imports` [EXTRACTED]
- [[ChildrenPage()]] - `contains` [EXTRACTED]
- [[ChildrenPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[DateTime()]] - `imports` [EXTRACTED]
- [[DateTime.tsx]] - `imports_from` [EXTRACTED]
- [[EmptyState()]] - `imports` [EXTRACTED]
- [[EmptyState.tsx]] - `imports_from` [EXTRACTED]
- [[ExemptionClaim]] - `contains` [EXTRACTED]
- [[ExemptionClaimForm()]] - `imports` [EXTRACTED]
- [[ExemptionClaimForm.tsx]] - `imports_from` [EXTRACTED]
- [[GuardianConsentSelector()]] - `imports` [EXTRACTED]
- [[GuardianConsentSelector.tsx]] - `imports_from` [EXTRACTED]
- [[GuardianForm()]] - `imports` [EXTRACTED]
- [[GuardianForm.tsx]] - `imports_from` [EXTRACTED]
- [[GuardianRelationship]] - `imports` [EXTRACTED]
- [[Input]] - `imports` [EXTRACTED]
- [[Label_1]] - `imports` [EXTRACTED]
- [[Notice]] - `imports` [EXTRACTED]
- [[NoticeDetail_1]] - `imports` [EXTRACTED]
- [[PrincipalListResponse]] - `contains` [EXTRACTED]
- [[Purpose]] - `contains` [EXTRACTED]
- [[Select]] - `imports` [EXTRACTED]
- [[Skeleton()]] - `imports` [EXTRACTED]
- [[Skeleton.tsx]] - `imports_from` [EXTRACTED]
- [[VerificationEditor()]] - `contains` [EXTRACTED]
- [[api-client.ts]] - `imports_from` [EXTRACTED]
- [[badge.tsx]] - `imports_from` [EXTRACTED]
- [[button.tsx]] - `imports_from` [EXTRACTED]
- [[card.tsx]] - `imports_from` [EXTRACTED]
- [[childrentypes.ts]] - `imports_from` [EXTRACTED]
- [[employeeApiClient]] - `imports` [EXTRACTED]
- [[guardianRowsForChild()]] - `contains` [EXTRACTED]
- [[guardianVerificationLabel()]] - `imports` [EXTRACTED]
- [[input.tsx]] - `imports_from` [EXTRACTED]
- [[isGuardianConsentEligible()]] - `imports` [EXTRACTED]
- [[label.tsx]] - `imports_from` [EXTRACTED]
- [[lucide-react_1]] - `imports_from` [EXTRACTED]
- [[noticestypes.ts]] - `imports_from` [EXTRACTED]
- [[react_1]] - `imports_from` [EXTRACTED]
- [[router.tsx]] - `imports_from` [EXTRACTED]
- [[select.tsx]] - `imports_from` [EXTRACTED]
- [[sonner_1]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/api-clientts
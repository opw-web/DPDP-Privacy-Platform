---
source_file: "demo-control/stage-demo.sh"
type: "code"
community: "common.sh"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/commonsh
---

# stage-demo.sh script

## Connections
- [[awareness_time()]] - `calls` [EXTRACTED]
- [[call()]] - `calls` [EXTRACTED]
- [[campaign_named()]] - `calls` [EXTRACTED]
- [[consent_as()]] - `calls` [EXTRACTED]
- [[counts()]] - `calls` [EXTRACTED]
- [[describe_purpose()]] - `calls` [EXTRACTED]
- [[failed()]] - `calls` [EXTRACTED]
- [[iso()]] - `calls` [EXTRACTED]
- [[jq_py()]] - `calls` [EXTRACTED]
- [[kept()]] - `calls` [EXTRACTED]
- [[login()]] - `calls` [EXTRACTED]
- [[mark()]] - `calls` [EXTRACTED]
- [[nid_of()]] - `calls` [EXTRACTED]
- [[ok()]] - `calls` [EXTRACTED]
- [[pause_before_exit()]] - `calls` [EXTRACTED]
- [[pcall()]] - `calls` [EXTRACTED]
- [[pid_of()]] - `calls` [EXTRACTED]
- [[plogin()]] - `calls` [EXTRACTED]
- [[publish_notice()]] - `calls` [EXTRACTED]
- [[say()]] - `calls` [EXTRACTED]
- [[setup_node()]] - `calls` [EXTRACTED]
- [[sid_of()]] - `calls` [EXTRACTED]
- [[stage-demo.sh]] - `contains` [EXTRACTED]
- [[step()]] - `calls` [EXTRACTED]
- [[tpl_of()]] - `calls` [EXTRACTED]
- [[warn()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/commonsh
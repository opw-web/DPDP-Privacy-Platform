---
source_file: "dpdp-platform/backend/src/common/prisma/scoped-transaction-client.ts"
type: "code"
community: ".record"
location: "L21"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/record
---

# ScopedTransactionClient

## Connections
- [[dot-applyMatch()]] - `references` [EXTRACTED]
- [[dot-assertGuardianConsentEligible()]] - `references` [EXTRACTED]
- [[dot-attachAvailableIdentifiers()]] - `references` [EXTRACTED]
- [[dot-attachIdentifier()]] - `references` [EXTRACTED]
- [[dot-buildChecklists()]] - `references` [EXTRACTED]
- [[dot-claimPendingCandidate()]] - `references` [EXTRACTED]
- [[dot-createCandidate()]] - `references` [EXTRACTED]
- [[dot-createFromTrigger()]] - `references` [EXTRACTED]
- [[dot-createPortalInTransaction()]] - `references` [EXTRACTED]
- [[dot-derive()]] - `references` [EXTRACTED]
- [[dot-findApplicableLegalHold()]] - `references` [EXTRACTED]
- [[dot-getOrCreateRecord()]] - `references` [EXTRACTED]
- [[dot-loadCandidate()]] - `references` [EXTRACTED]
- [[dot-loadProfile()]] - `references` [EXTRACTED]
- [[dot-match()]] - `references` [EXTRACTED]
- [[dot-maybeFinalizeCampaign()]] - `references` [EXTRACTED]
- [[dot-mergeRecordIntoPrincipal()]] - `references` [EXTRACTED]
- [[dot-nextPrincipalReferenceInTransaction()]] - `references` [EXTRACTED]
- [[dot-persistAndLink()]] - `references` [EXTRACTED]
- [[dot-rebuild()]] - `references` [EXTRACTED]
- [[dot-record()]] - `references` [EXTRACTED]
- [[dot-record()_1]] - `references` [EXTRACTED]
- [[dot-recordPersonalDataViewed()]] - `references` [EXTRACTED]
- [[dot-rescrubFieldSample()]] - `references` [EXTRACTED]
- [[dot-resolveFieldsInTransaction()]] - `references` [EXTRACTED]
- [[dot-resolveLastInboundContactAt()]] - `references` [EXTRACTED]
- [[dot-resolveLastProcessingAt()]] - `references` [EXTRACTED]
- [[dot-resolveSignal()]] - `references` [EXTRACTED]
- [[dot-supportingCandidates()]] - `references` [EXTRACTED]
- [[dot-verifyScopedReferences()]] - `references` [EXTRACTED]
- [[dot-writeTransition()]] - `references` [EXTRACTED]
- [[access-log.service.ts]] - `imports` [EXTRACTED]
- [[age.service.ts]] - `imports` [EXTRACTED]
- [[assembly.service.ts]] - `imports` [EXTRACTED]
- [[audit.service.ts]] - `imports` [EXTRACTED]
- [[boardnon-disclosure.ts]] - `imports` [EXTRACTED]
- [[campaigns.service.ts]] - `imports` [EXTRACTED]
- [[candidates.service.ts]] - `imports` [EXTRACTED]
- [[consents.service.ts]] - `imports` [EXTRACTED]
- [[counter.ts]] - `imports` [EXTRACTED]
- [[data-sources.service.ts]] - `imports` [EXTRACTED]
- [[erasure-task.service.ts]] - `imports` [EXTRACTED]
- [[evidencenon-disclosure.ts]] - `imports` [EXTRACTED]
- [[guardians.service.ts]] - `imports` [EXTRACTED]
- [[identifier-ownership-lock.ts]] - `imports` [EXTRACTED]
- [[linking.service.ts]] - `imports` [EXTRACTED]
- [[mapping-warnings.ts]] - `imports` [EXTRACTED]
- [[matching.service.ts]] - `imports` [EXTRACTED]
- [[merge.service.ts]] - `imports` [EXTRACTED]
- [[notifications.service.ts]] - `imports` [EXTRACTED]
- [[principals.service.ts]] - `imports` [EXTRACTED]
- [[purpose-served.service.ts]] - `imports` [EXTRACTED]
- [[reference.service.ts]] - `imports` [EXTRACTED]
- [[refresh-rotation.util.ts]] - `imports` [EXTRACTED]
- [[requests.service.ts]] - `imports` [EXTRACTED]
- [[retention-transaction-lock.util.ts]] - `imports` [EXTRACTED]
- [[scoped-transaction-client.ts]] - `contains` [EXTRACTED]
- [[sync-pipeline.service.ts]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/record
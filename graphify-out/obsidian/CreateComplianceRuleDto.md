---
source_file: "dpdp-platform/backend/src/modules/compliance/dto/create-compliance-rule.dto.ts"
type: "code"
community: "CreateComplianceRuleDto"
location: "L21"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/CreateComplianceRuleDto
---

# CreateComplianceRuleDto

## Connections
- [[dot-create()_1]] - `references` [EXTRACTED]
- [[dot-create()_12]] - `references` [EXTRACTED]
- [[ApiProperty_50]] - `references` [EXTRACTED]
- [[ApiPropertyOptional_53]] - `references` [EXTRACTED]
- [[IsBoolean_27]] - `references` [EXTRACTED]
- [[IsDateString_23]] - `references` [EXTRACTED]
- [[IsEnum_23]] - `references` [EXTRACTED]
- [[IsInt_11]] - `references` [EXTRACTED]
- [[IsOptional_51]] - `references` [EXTRACTED]
- [[IsString_69]] - `references` [EXTRACTED]
- [[Min_11]] - `references` [EXTRACTED]
- [[MinLength_54]] - `references` [EXTRACTED]
- [[compliance.controller.ts]] - `imports` [EXTRACTED]
- [[compliance.service.ts]] - `imports` [EXTRACTED]
- [[create-compliance-rule.dto.ts]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/CreateComplianceRuleDto
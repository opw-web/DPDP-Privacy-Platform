---
source_file: "dpdp-platform/backend/src/modules/children/guardians.service.ts"
type: "code"
community: "guardians.service.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/guardiansservicets
---

# guardians.service.ts

## Connections
- [[APPOINTING_AUTHORITIES]] - `imports` [EXTRACTED]
- [[AccessTokenPayload]] - `imports` [EXTRACTED]
- [[AuditService]] - `imports` [EXTRACTED]
- [[CHILD_LIKE_AGE_STATUSES]] - `contains` [EXTRACTED]
- [[CreateGuardianDto]] - `imports` [EXTRACTED]
- [[GUARDIAN_PUBLIC_SELECT]] - `contains` [EXTRACTED]
- [[GuardianRow]] - `contains` [EXTRACTED]
- [[GuardiansService]] - `contains` [EXTRACTED]
- [[ListGuardiansDto]] - `imports` [EXTRACTED]
- [[MaskingService]] - `imports` [EXTRACTED]
- [[PrismaService]] - `imports` [EXTRACTED]
- [[PublicGuardian]] - `contains` [EXTRACTED]
- [[ScopedTransactionClient]] - `imports` [EXTRACTED]
- [[TenantScopedPrismaClient]] - `imports` [EXTRACTED]
- [[VerifyGuardianDto]] - `imports` [EXTRACTED]
- [[appointing-authority.ts]] - `imports_from` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[children.e2e-spec.ts]] - `imports_from` [EXTRACTED]
- [[children.module.ts]] - `imports_from` [EXTRACTED]
- [[consents.service.ts]] - `imports_from` [EXTRACTED]
- [[create-guardian.dto.ts]] - `imports_from` [EXTRACTED]
- [[guardians.controller.ts]] - `imports_from` [EXTRACTED]
- [[isPwd()]] - `contains` [EXTRACTED]
- [[list-guardians.dto.ts]] - `imports_from` [EXTRACTED]
- [[masking.service.ts]] - `imports_from` [EXTRACTED]
- [[prisma.service.ts]] - `imports_from` [EXTRACTED]
- [[scoped-transaction-client.ts]] - `imports_from` [EXTRACTED]
- [[toPublicGuardian()]] - `contains` [EXTRACTED]
- [[token.service.ts]] - `imports_from` [EXTRACTED]
- [[verify-guardian.dto.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/guardiansservicets
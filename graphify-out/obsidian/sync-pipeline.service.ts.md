---
source_file: "dpdp-platform/backend/src/modules/sync/sync-pipeline.service.ts"
type: "code"
community: "sync-pipeline.service.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/sync-pipelineservicets
---

# sync-pipeline.service.ts

## Connections
- [[AgeService]] - `imports` [EXTRACTED]
- [[AssemblyService]] - `imports` [EXTRACTED]
- [[AuditService]] - `imports` [EXTRACTED]
- [[DataSourcesService]] - `imports` [EXTRACTED]
- [[LinkingService]] - `imports` [EXTRACTED]
- [[MatchingService]] - `imports` [EXTRACTED]
- [[MissingRecordKeyError]] - `imports` [EXTRACTED]
- [[NormalizationMapping]] - `imports` [EXTRACTED]
- [[NormalizationService]] - `imports` [EXTRACTED]
- [[PrismaService]] - `imports` [EXTRACTED]
- [[RecordContext]] - `contains` [EXTRACTED]
- [[ScopedTransactionClient]] - `imports` [EXTRACTED]
- [[SyncCounts]] - `contains` [EXTRACTED]
- [[SyncErrorLogEntry]] - `contains` [EXTRACTED]
- [[SyncLockService]] - `imports` [EXTRACTED]
- [[SyncLockUnavailableError]] - `imports` [EXTRACTED]
- [[SyncPipelineService]] - `contains` [EXTRACTED]
- [[SyncRunSummary]] - `contains` [EXTRACTED]
- [[TenantContext]] - `imports` [EXTRACTED]
- [[TenantStore]] - `imports` [EXTRACTED]
- [[age.service.ts]] - `imports_from` [EXTRACTED]
- [[assembly.service.ts]] - `imports_from` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[buildCandidateSignals()]] - `imports` [EXTRACTED]
- [[data-sources.service.ts]] - `imports_from` [EXTRACTED]
- [[describeSyncError()]] - `imports` [EXTRACTED]
- [[extractRecordKey()]] - `contains` [EXTRACTED]
- [[hashPayload()]] - `imports` [EXTRACTED]
- [[identifier-ownership-lock.ts]] - `imports_from` [EXTRACTED]
- [[linking.service.ts]] - `imports_from` [EXTRACTED]
- [[lockIdentifiersForOwnership()]] - `imports` [EXTRACTED]
- [[lockNameKeyForOwnership()]] - `imports` [EXTRACTED]
- [[matching.service.ts]] - `imports_from` [EXTRACTED]
- [[normalization.service.ts]] - `imports_from` [EXTRACTED]
- [[payload-hash.ts]] - `imports_from` [EXTRACTED]
- [[prisma.service.ts]] - `imports_from` [EXTRACTED]
- [[scoped-transaction-client.ts]] - `imports_from` [EXTRACTED]
- [[step6-conflict-acceptance.e2e-spec.ts]] - `imports_from` [EXTRACTED]
- [[sync-error.ts]] - `imports_from` [EXTRACTED]
- [[sync-lock.service.ts]] - `imports_from` [EXTRACTED]
- [[sync.e2e-spec.ts]] - `imports_from` [EXTRACTED]
- [[sync.module.ts]] - `imports_from` [EXTRACTED]
- [[sync.processor.ts]] - `imports_from` [EXTRACTED]
- [[tenant-context.ts]] - `imports_from` [EXTRACTED]
- [[zeroCounts()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/sync-pipelineservicets
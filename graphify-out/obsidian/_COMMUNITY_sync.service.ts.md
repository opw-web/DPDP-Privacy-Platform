---
type: community
members: 33
---

# sync.service.ts

**Members:** 33 nodes

## Members
- [[dot-constructor()_39]] - code - dpdp-platform/backend/src/modules/sync/sync.controller.ts
- [[dot-constructor()_40]] - code - dpdp-platform/backend/src/modules/sync/sync.controller.ts
- [[dot-get()_5]] - code - dpdp-platform/backend/src/modules/data-sources/data-sources.service.ts
- [[dot-get()_6]] - code - dpdp-platform/backend/src/modules/sync/sync.controller.ts
- [[dot-getJob()]] - code - dpdp-platform/backend/src/modules/sync/sync.service.ts
- [[dot-list()_7]] - code - dpdp-platform/backend/src/modules/sync/sync.controller.ts
- [[dot-listJobs()]] - code - dpdp-platform/backend/src/modules/sync/sync.service.ts
- [[dot-trigger()_1]] - code - dpdp-platform/backend/src/modules/sync/sync.controller.ts
- [[dot-triggerSync()]] - code - dpdp-platform/backend/src/modules/sync/sync.service.ts
- [[ApiTags_7]] - code
- [[Controller_8]] - code
- [[DEFAULT_SYNC_JOB_LIST_LIMIT]] - code - dpdp-platform/backend/src/modules/sync/dto/list-sync-jobs.dto.ts
- [[Get_8]] - code
- [[HttpCode]] - code
- [[Injectable_26]] - code
- [[IsInt_1]] - code
- [[IsOptional_11]] - code
- [[IsString_12]] - code
- [[ListSyncJobsQueryDto]] - code - dpdp-platform/backend/src/modules/sync/dto/list-sync-jobs.dto.ts
- [[MAX_SYNC_JOB_LIST_LIMIT]] - code - dpdp-platform/backend/src/modules/sync/dto/list-sync-jobs.dto.ts
- [[Max]] - code
- [[Min_1]] - code
- [[Param_5]] - code
- [[Post_5]] - code
- [[Query_1]] - code
- [[SyncController]] - code - dpdp-platform/backend/src/modules/sync/sync.controller.ts
- [[SyncJobsController]] - code - dpdp-platform/backend/src/modules/sync/sync.controller.ts
- [[SyncService]] - code - dpdp-platform/backend/src/modules/sync/sync.service.ts
- [[TriggerSyncResult]] - code - dpdp-platform/backend/src/modules/sync/sync.service.ts
- [[Type_1]] - code
- [[list-sync-jobs.dto.ts]] - code - dpdp-platform/backend/src/modules/sync/dto/list-sync-jobs.dto.ts
- [[sync.controller.ts]] - code - dpdp-platform/backend/src/modules/sync/sync.controller.ts
- [[sync.service.ts]] - code - dpdp-platform/backend/src/modules/sync/sync.service.ts

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/syncservicets
SORT file.name ASC
```

## Connections to other communities
- 5 edges to [[_COMMUNITY_app.module.ts]]
- 5 edges to [[_COMMUNITY_RequirePermission]]
- 3 edges to [[_COMMUNITY_DataSourcesService]]
- 3 edges to [[_COMMUNITY_prisma.service.ts]]
- 2 edges to [[_COMMUNITY_SyncQueueService]]
- 2 edges to [[_COMMUNITY_queues.module.ts]]
- 1 edge to [[_COMMUNITY_PrismaService]]
- 1 edge to [[_COMMUNITY_data-sources.service.ts]]

## Top bridge nodes
- [[sync.service.ts]] - degree 15, connects to 7 communities
- [[SyncService]] - degree 10, connects to 2 communities
- [[sync.controller.ts]] - degree 9, connects to 2 communities
- [[SyncJobsController]] - degree 7, connects to 1 community
- [[SyncController]] - degree 6, connects to 1 community
---
source_file: "dpdp-platform/backend/src/modules/identity/merge.service.ts"
type: "code"
community: "prisma.service.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/prismaservicets
---

# merge.service.ts

## Connections
- [[@nestjscommon_1]] - `imports_from` [EXTRACTED]
- [[@prismaclient_1]] - `imports_from` [EXTRACTED]
- [[AccessTokenPayload]] - `imports` [EXTRACTED]
- [[AgeService]] - `imports` [EXTRACTED]
- [[AssemblyService]] - `imports` [EXTRACTED]
- [[AuditService]] - `imports` [EXTRACTED]
- [[MergeParams]] - `contains` [EXTRACTED]
- [[MergeResult]] - `contains` [EXTRACTED]
- [[MergeService]] - `contains` [EXTRACTED]
- [[PrismaService]] - `imports` [EXTRACTED]
- [[ReferenceService]] - `imports` [EXTRACTED]
- [[ScopedTransactionClient]] - `imports` [EXTRACTED]
- [[UnmergeResult]] - `contains` [EXTRACTED]
- [[age.service.ts]] - `imports_from` [EXTRACTED]
- [[assembly.service.ts]] - `imports_from` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[candidates.service.ts]] - `imports_from` [EXTRACTED]
- [[identity.module.ts]] - `imports_from` [EXTRACTED]
- [[initialPrincipalDisplayName()]] - `imports` [EXTRACTED]
- [[linking.service.ts]] - `imports_from` [EXTRACTED]
- [[merge-unmerge.e2e-spec.ts]] - `imports_from` [EXTRACTED]
- [[prisma.service.ts]] - `imports_from` [EXTRACTED]
- [[reference.service.ts]] - `imports_from` [EXTRACTED]
- [[scoped-transaction-client.ts]] - `imports_from` [EXTRACTED]
- [[token.service.ts]] - `imports_from` [EXTRACTED]
- [[unmerge.controller.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/prismaservicets
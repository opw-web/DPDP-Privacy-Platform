---
source_file: "dpdp-platform/backend/src/modules/registers/sharing.service.ts"
type: "code"
community: "CreateSharingActivityDto"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/CreateSharingActivityDto
---

# sharing.service.ts

## Connections
- [[AuditService]] - `imports` [EXTRACTED]
- [[CreateSharingActivityDto]] - `imports` [EXTRACTED]
- [[PrismaService]] - `imports` [EXTRACTED]
- [[PublicSharingActivity]] - `contains` [EXTRACTED]
- [[SHARING_ACTIVITY_PUBLIC_SELECT]] - `contains` [EXTRACTED]
- [[SharingService]] - `contains` [EXTRACTED]
- [[UpdateSharingActivityDto]] - `imports` [EXTRACTED]
- [[assertNonBlankDescription()]] - `contains` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[create-sharing-activity.dto.ts]] - `imports_from` [EXTRACTED]
- [[prisma.service.ts]] - `imports_from` [EXTRACTED]
- [[registers.module.ts]] - `imports_from` [EXTRACTED]
- [[sharing.controller.ts]] - `imports_from` [EXTRACTED]
- [[update-sharing-activity.dto.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/CreateSharingActivityDto
---
source_file: "dpdp-platform/frontend/src/lib/auth.ts"
type: "code"
community: "router.tsx"
location: "L126"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/routertsx
---

# employeeLogin()

## Connections
- [[AuditPage.test.tsx]] - `imports` [EXTRACTED]
- [[DashboardPage.test.tsx]] - `imports` [EXTRACTED]
- [[DataSourceDetailPage.test.tsx]] - `imports` [EXTRACTED]
- [[DataSourceNewPage.test.tsx]] - `imports` [EXTRACTED]
- [[EmployeesPage.test.tsx]] - `imports` [EXTRACTED]
- [[InformationRequestsPage.test.tsx]] - `imports` [EXTRACTED]
- [[LinkedRecordsPanel.test.tsx]] - `imports` [EXTRACTED]
- [[LoginPage()]] - `calls` [EXTRACTED]
- [[LoginPage.tsx]] - `imports` [EXTRACTED]
- [[MessagingCampaignDetailPage.test.tsx]] - `imports` [EXTRACTED]
- [[NoticeBuilderPage.test.tsx]] - `imports` [EXTRACTED]
- [[PrincipalDetailPage.test.tsx]] - `imports` [EXTRACTED]
- [[PrincipalEvidencePage.test.tsx]] - `imports` [EXTRACTED]
- [[PrincipalsPage.test.tsx]] - `imports` [EXTRACTED]
- [[PurposesPage.test.tsx]] - `imports` [EXTRACTED]
- [[RecipientsTab.test.tsx]] - `imports` [EXTRACTED]
- [[RequestDetailPage.test.tsx]] - `imports` [EXTRACTED]
- [[ReviewQueuePage.test.tsx]] - `imports` [EXTRACTED]
- [[SdfPage.test.tsx]] - `imports` [EXTRACTED]
- [[SettingsCompliancePage.test.tsx]] - `imports` [EXTRACTED]
- [[SettingsPage.test.tsx]] - `imports` [EXTRACTED]
- [[api-client.test.ts]] - `imports` [EXTRACTED]
- [[fetchEmployeeMe()]] - `calls` [EXTRACTED]
- [[libauth.ts]] - `contains` [EXTRACTED]
- [[loginAndRender()_1]] - `calls` [EXTRACTED]
- [[loginAndRender()_2]] - `calls` [EXTRACTED]
- [[loginAndRender()_3]] - `calls` [EXTRACTED]
- [[loginAndRender()_4]] - `calls` [EXTRACTED]
- [[loginAndRender()_5]] - `calls` [EXTRACTED]
- [[loginAndRender()_6]] - `calls` [EXTRACTED]
- [[loginAndRenderDetailPage()]] - `calls` [EXTRACTED]
- [[loginAndRenderThroughShell()]] - `calls` [EXTRACTED]
- [[loginAndRenderThroughShell()_1]] - `calls` [EXTRACTED]
- [[loginAndRenderThroughShell()_2]] - `calls` [EXTRACTED]
- [[loginAndRenderThroughShell()_3]] - `calls` [EXTRACTED]
- [[loginAndRenderWizard()]] - `calls` [EXTRACTED]
- [[mockRoutesAndLogin()]] - `calls` [EXTRACTED]
- [[renderDetail()]] - `calls` [EXTRACTED]
- [[renderPage()_6]] - `calls` [EXTRACTED]
- [[renderPage()_7]] - `calls` [EXTRACTED]
- [[router.test.tsx]] - `imports` [EXTRACTED]
- [[useEmployeeAuth()]] - `indirect_call` [INFERRED]

#graphify/code #graphify/EXTRACTED #community/routertsx
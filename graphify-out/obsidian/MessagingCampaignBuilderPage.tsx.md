---
source_file: "dpdp-platform/frontend/src/fiduciary/pages/MessagingCampaignBuilderPage.tsx"
type: "code"
community: "MessagingCampaignBuilderPage.tsx"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/MessagingCampaignBuilderPagetsx
---

# MessagingCampaignBuilderPage.tsx

## Connections
- [[@tanstackreact-query_1]] - `imports_from` [EXTRACTED]
- [[AudienceBuilder()]] - `imports` [EXTRACTED]
- [[AudienceBuilder.tsx]] - `imports_from` [EXTRACTED]
- [[AudienceFilter_1]] - `imports` [EXTRACTED]
- [[AudiencePreview]] - `imports` [EXTRACTED]
- [[Button]] - `imports` [EXTRACTED]
- [[CATEGORIES]] - `contains` [EXTRACTED]
- [[CampaignCategory]] - `contains` [EXTRACTED]
- [[CampaignDraft]] - `contains` [EXTRACTED]
- [[CampaignTemplate]] - `contains` [EXTRACTED]
- [[Card]] - `imports` [EXTRACTED]
- [[CardContent]] - `imports` [EXTRACTED]
- [[CardHeader]] - `imports` [EXTRACTED]
- [[CardTitle]] - `imports` [EXTRACTED]
- [[MarkdownMessageEditor()]] - `imports` [EXTRACTED]
- [[MarkdownMessageEditor.tsx]] - `imports_from` [EXTRACTED]
- [[MessagingCampaignBuilderPage()]] - `contains` [EXTRACTED]
- [[MessagingCampaignBuilderPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[Notice]] - `imports` [EXTRACTED]
- [[api-client.ts]] - `imports_from` [EXTRACTED]
- [[breach-notification-elements.ts]] - `imports_from` [EXTRACTED]
- [[buildCampaignPayload()]] - `contains` [EXTRACTED]
- [[button.tsx]] - `imports_from` [EXTRACTED]
- [[campaignConfirmationText()]] - `contains` [EXTRACTED]
- [[card.tsx]] - `imports_from` [EXTRACTED]
- [[consentRequestSubmissionBlocked()]] - `contains` [EXTRACTED]
- [[employeeApiClient]] - `imports` [EXTRACTED]
- [[marketingSubmissionBlocked()]] - `contains` [EXTRACTED]
- [[missingBreachPlaceholders()]] - `imports` [EXTRACTED]
- [[missingPlaceholdersKey()]] - `imports` [EXTRACTED]
- [[noticestypes.ts]] - `imports_from` [EXTRACTED]
- [[pickDefaultBreachTemplate()]] - `contains` [EXTRACTED]
- [[react_1]] - `imports_from` [EXTRACTED]
- [[react-router-dom_1]] - `imports_from` [EXTRACTED]
- [[router.tsx]] - `imports_from` [EXTRACTED]
- [[sonner_1]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/MessagingCampaignBuilderPagetsx
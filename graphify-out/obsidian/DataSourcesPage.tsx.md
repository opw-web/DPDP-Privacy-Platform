---
source_file: "dpdp-platform/frontend/src/fiduciary/pages/DataSourcesPage.tsx"
type: "code"
community: "DataSourceDetailPage.tsx"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/DataSourceDetailPagetsx
---

# DataSourcesPage.tsx

## Connections
- [[ApiError]] - `imports` [EXTRACTED]
- [[Badge()]] - `imports` [EXTRACTED]
- [[Button]] - `imports` [EXTRACTED]
- [[DataSourceStatus]] - `imports` [EXTRACTED]
- [[DataSourcesPage()]] - `contains` [EXTRACTED]
- [[DataTable()]] - `imports` [EXTRACTED]
- [[DataTable.tsx]] - `imports_from` [EXTRACTED]
- [[DateTime()]] - `imports` [EXTRACTED]
- [[DateTime.tsx]] - `imports_from` [EXTRACTED]
- [[PermissionGate()]] - `imports` [EXTRACTED]
- [[PermissionGate.tsx]] - `imports_from` [EXTRACTED]
- [[PublicDataSource]] - `imports` [EXTRACTED]
- [[STATUS_VARIANT_1]] - `contains` [EXTRACTED]
- [[SyncJob]] - `imports` [EXTRACTED]
- [[SyncNowButton()]] - `contains` [EXTRACTED]
- [[SyncNowButtonProps]] - `contains` [EXTRACTED]
- [[api-client.ts]] - `imports_from` [EXTRACTED]
- [[badge.tsx]] - `imports_from` [EXTRACTED]
- [[button.tsx]] - `imports_from` [EXTRACTED]
- [[data-sources-api.ts]] - `imports_from` [EXTRACTED]
- [[employeeApiClient]] - `imports` [EXTRACTED]
- [[router.tsx]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/DataSourceDetailPagetsx
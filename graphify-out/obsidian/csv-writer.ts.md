---
source_file: "dpdp-platform/backend/src/modules/inventory/csv-writer.ts"
type: "code"
community: "evidence-pack.service.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/evidence-packservicets
---

# csv-writer.ts

## Connections
- [[RFC-4180_1]] - `cites` [EXTRACTED]
- [[access-report-render.ts]] - `re_exports` [EXTRACTED]
- [[audit-export.service.ts]] - `imports_from` [EXTRACTED]
- [[audit-read.service.ts]] - `imports_from` [EXTRACTED]
- [[csv-writer.spec.ts]] - `imports_from` [EXTRACTED]
- [[csvDocument()]] - `contains` [EXTRACTED]
- [[csvField()]] - `contains` [EXTRACTED]
- [[csvRow()]] - `contains` [EXTRACTED]
- [[evidence-pack.service.ts]] - `imports_from` [EXTRACTED]
- [[needsQuoting()]] - `contains` [EXTRACTED]
- [[pack-letterhead.ts]] - `imports_from` [EXTRACTED]
- [[ropa-export.service.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/evidence-packservicets
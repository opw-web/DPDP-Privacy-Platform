---
source_file: "dpdp-platform/frontend/src/fiduciary/components/sdf/AssessmentRow.tsx"
type: "code"
community: "react"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/react
---

# AssessmentRow.tsx

## Connections
- [[@tanstackreact-query_1]] - `imports_from` [EXTRACTED]
- [[ApiError]] - `imports` [EXTRACTED]
- [[AssessmentRow()]] - `contains` [EXTRACTED]
- [[AssessmentRowData]] - `contains` [EXTRACTED]
- [[Badge()]] - `imports` [EXTRACTED]
- [[Button]] - `imports` [EXTRACTED]
- [[CheckboxOption]] - `imports` [EXTRACTED]
- [[CompleteFormValues]] - `contains` [EXTRACTED]
- [[DeadlinePill()]] - `imports` [EXTRACTED]
- [[DeadlinePill.tsx]] - `imports_from` [EXTRACTED]
- [[Input]] - `imports` [EXTRACTED]
- [[Label_1]] - `imports` [EXTRACTED]
- [[SdfPage.tsx]] - `imports_from` [EXTRACTED]
- [[Textarea]] - `imports` [EXTRACTED]
- [[api-client.ts]] - `imports_from` [EXTRACTED]
- [[badge.tsx]] - `imports_from` [EXTRACTED]
- [[button.tsx]] - `imports_from` [EXTRACTED]
- [[checkbox.tsx]] - `imports_from` [EXTRACTED]
- [[describeCompleteError()]] - `contains` [EXTRACTED]
- [[employeeApiClient]] - `imports` [EXTRACTED]
- [[input.tsx]] - `imports_from` [EXTRACTED]
- [[label.tsx]] - `imports_from` [EXTRACTED]
- [[react_1]] - `imports_from` [EXTRACTED]
- [[sonner_1]] - `imports_from` [EXTRACTED]
- [[textarea.tsx]] - `imports_from` [EXTRACTED]
- [[toCompletePayload()]] - `contains` [EXTRACTED]
- [[toDatetimeLocalValue()]] - `contains` [EXTRACTED]
- [[toFormValues()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/react
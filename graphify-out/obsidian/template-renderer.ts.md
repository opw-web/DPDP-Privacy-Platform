---
source_file: "dpdp-platform/backend/src/modules/messaging/templates/template-renderer.ts"
type: "code"
community: "campaigns.service.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/campaignsservicets
---

# template-renderer.ts

## Connections
- [[BREACH_NOTIFICATION_REQUIRED_ELEMENTS]] - `imports` [EXTRACTED]
- [[DisallowedTemplateSyntaxError]] - `contains` [EXTRACTED]
- [[MissingOrganizationContactError]] - `contains` [EXTRACTED]
- [[MissingRequiredVariableError]] - `contains` [EXTRACTED]
- [[OrganizationContactFields]] - `contains` [EXTRACTED]
- [[RenderOrganizationTemplateInput]] - `contains` [EXTRACTED]
- [[RenderTemplateInput]] - `contains` [EXTRACTED]
- [[RenderedTemplate]] - `contains` [EXTRACTED]
- [[TemplateRenderError]] - `contains` [EXTRACTED]
- [[TemplateVariableName]] - `imports` [EXTRACTED]
- [[UnknownTemplateVariableError]] - `contains` [EXTRACTED]
- [[assertSimpleWhitelistedMustache()]] - `contains` [EXTRACTED]
- [[campaigns.service.ts]] - `imports_from` [EXTRACTED]
- [[engine]] - `contains` [EXTRACTED]
- [[extractTemplateVariables()]] - `contains` [EXTRACTED]
- [[handlebars_1]] - `imports_from` [EXTRACTED]
- [[isWhitelistedTemplateVariable()]] - `imports` [EXTRACTED]
- [[message-templates.ts]] - `imports_from` [EXTRACTED]
- [[renderMessageTemplate()]] - `contains` [EXTRACTED]
- [[renderOrganizationMessageTemplate()]] - `contains` [EXTRACTED]
- [[renderSource()]] - `contains` [EXTRACTED]
- [[resolveOrganizationContactVariables()]] - `contains` [EXTRACTED]
- [[template-renderer.spec.ts]] - `imports_from` [EXTRACTED]
- [[templates.service.ts]] - `imports_from` [EXTRACTED]
- [[walkStatement()]] - `contains` [EXTRACTED]
- [[whitelisted-variables.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/campaignsservicets
---
source_file: "dpdp-platform/backend/src/modules/data-sources/data-sources.service.ts"
type: "code"
community: "audit.service.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/auditservicets
---

# data-sources.service.ts

## Connections
- [[@nestjscommon_1]] - `imports_from` [EXTRACTED]
- [[@prismaclient_1]] - `imports_from` [EXTRACTED]
- [[AuditService]] - `imports` [EXTRACTED]
- [[CONNECTOR_SOURCE_SELECT]] - `contains` [EXTRACTED]
- [[Connector]] - `imports` [EXTRACTED]
- [[ConnectorFactory]] - `imports` [EXTRACTED]
- [[CreateDataSourceDto]] - `imports` [EXTRACTED]
- [[CryptoService]] - `imports` [EXTRACTED]
- [[DATA_SOURCE_FIELD_SELECT]] - `contains` [EXTRACTED]
- [[DATA_SOURCE_PUBLIC_SELECT]] - `contains` [EXTRACTED]
- [[DataSourceRowForConnector]] - `imports` [EXTRACTED]
- [[DataSourcesService]] - `contains` [EXTRACTED]
- [[PrismaService]] - `imports` [EXTRACTED]
- [[PublicDataSource_1]] - `contains` [EXTRACTED]
- [[PublicDataSourceField_1]] - `contains` [EXTRACTED]
- [[ScopedTransactionClient]] - `imports` [EXTRACTED]
- [[SyncQueueService]] - `imports` [EXTRACTED]
- [[TenantScopedPrismaClient]] - `imports` [EXTRACTED]
- [[TestConnectionResult_1]] - `contains` [EXTRACTED]
- [[UpdateDataSourceDto]] - `imports` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[connector.factory.ts]] - `imports_from` [EXTRACTED]
- [[connector.interface.ts]] - `imports_from` [EXTRACTED]
- [[create-data-source.dto.ts]] - `imports_from` [EXTRACTED]
- [[crypto.service.ts]] - `imports_from` [EXTRACTED]
- [[data-sources.controller.ts]] - `imports_from` [EXTRACTED]
- [[data-sources.e2e-spec.ts]] - `imports_from` [EXTRACTED]
- [[data-sources.module.ts]] - `imports_from` [EXTRACTED]
- [[duplicateNameMessage()]] - `contains` [EXTRACTED]
- [[isUniqueConstraintViolation()_2]] - `contains` [EXTRACTED]
- [[mappings.service.ts]] - `imports_from` [EXTRACTED]
- [[prisma.service.ts]] - `imports_from` [EXTRACTED]
- [[scoped-transaction-client.ts]] - `imports_from` [EXTRACTED]
- [[sync-pipeline.service.ts]] - `imports_from` [EXTRACTED]
- [[sync.queue.ts]] - `imports_from` [EXTRACTED]
- [[sync.service.ts]] - `imports_from` [EXTRACTED]
- [[update-data-source.dto.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/auditservicets
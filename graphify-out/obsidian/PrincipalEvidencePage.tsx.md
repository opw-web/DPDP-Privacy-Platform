---
source_file: "dpdp-platform/frontend/src/fiduciary/pages/PrincipalEvidencePage.tsx"
type: "code"
community: "AuditPage.tsx"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/AuditPagetsx
---

# PrincipalEvidencePage.tsx

## Connections
- [[@tanstackreact-query_1]] - `imports_from` [EXTRACTED]
- [[ApiError]] - `imports` [EXTRACTED]
- [[Badge()]] - `imports` [EXTRACTED]
- [[Button]] - `imports` [EXTRACTED]
- [[Card]] - `imports` [EXTRACTED]
- [[CardContent]] - `imports` [EXTRACTED]
- [[CardDescription]] - `imports` [EXTRACTED]
- [[CardHeader]] - `imports` [EXTRACTED]
- [[CardTitle]] - `imports` [EXTRACTED]
- [[DateTime()]] - `imports` [EXTRACTED]
- [[DateTime.tsx]] - `imports_from` [EXTRACTED]
- [[DownloadButton()]] - `contains` [EXTRACTED]
- [[EmptyEvidence()]] - `contains` [EXTRACTED]
- [[EmptyState()]] - `imports` [EXTRACTED]
- [[EmptyState.tsx]] - `imports_from` [EXTRACTED]
- [[EvidenceCard()]] - `contains` [EXTRACTED]
- [[PermissionGate()]] - `imports` [EXTRACTED]
- [[PermissionGate.tsx]] - `imports_from` [EXTRACTED]
- [[PrincipalEvidenceBreachInclusion]] - `contains` [EXTRACTED]
- [[PrincipalEvidenceConsentEvent]] - `contains` [EXTRACTED]
- [[PrincipalEvidenceFile_1]] - `contains` [EXTRACTED]
- [[PrincipalEvidenceMessage]] - `contains` [EXTRACTED]
- [[PrincipalEvidenceNoticeVersion]] - `contains` [EXTRACTED]
- [[PrincipalEvidencePage()]] - `contains` [EXTRACTED]
- [[PrincipalEvidencePage.test.tsx]] - `imports_from` [EXTRACTED]
- [[PrincipalEvidenceRequest]] - `contains` [EXTRACTED]
- [[PrincipalEvidenceRequestEvent]] - `contains` [EXTRACTED]
- [[Skeleton()]] - `imports` [EXTRACTED]
- [[Skeleton.tsx]] - `imports_from` [EXTRACTED]
- [[Table]] - `imports` [EXTRACTED]
- [[TableBody]] - `imports` [EXTRACTED]
- [[TableCell]] - `imports` [EXTRACTED]
- [[TableHead]] - `imports` [EXTRACTED]
- [[TableHeader]] - `imports` [EXTRACTED]
- [[TableRow]] - `imports` [EXTRACTED]
- [[api-client.ts]] - `imports_from` [EXTRACTED]
- [[badge.tsx]] - `imports_from` [EXTRACTED]
- [[button.tsx]] - `imports_from` [EXTRACTED]
- [[card.tsx]] - `imports_from` [EXTRACTED]
- [[employeeApiClient]] - `imports` [EXTRACTED]
- [[enum-options.ts]] - `imports_from` [EXTRACTED]
- [[humanizeEnum()]] - `imports` [EXTRACTED]
- [[lucide-react_1]] - `imports_from` [EXTRACTED]
- [[react_1]] - `imports_from` [EXTRACTED]
- [[react-router-dom_1]] - `imports_from` [EXTRACTED]
- [[router.tsx]] - `imports_from` [EXTRACTED]
- [[saveBlob()_1]] - `contains` [EXTRACTED]
- [[sonner_1]] - `imports_from` [EXTRACTED]
- [[table.tsx]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/AuditPagetsx
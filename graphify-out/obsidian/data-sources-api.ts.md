---
source_file: "dpdp-platform/frontend/src/fiduciary/lib/data-sources-api.ts"
type: "code"
community: "data-sources-api.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/data-sources-apits
---

# data-sources-api.ts

## Connections
- [[API_BASE]] - `imports` [EXTRACTED]
- [[AUTH_TYPE_VALUES]] - `contains` [EXTRACTED]
- [[ApiError]] - `imports` [EXTRACTED]
- [[AttachedPurpose]] - `contains` [EXTRACTED]
- [[AuthType]] - `contains` [EXTRACTED]
- [[BreachWizardPage.tsx]] - `imports_from` [EXTRACTED]
- [[CANONICAL_FIELD_VALUES]] - `contains` [EXTRACTED]
- [[CanonicalField]] - `contains` [EXTRACTED]
- [[DATA_CATEGORY_VALUES]] - `contains` [EXTRACTED]
- [[DataCategory]] - `contains` [EXTRACTED]
- [[DataSourceDetailPage.tsx]] - `imports_from` [EXTRACTED]
- [[DataSourceNewPage.tsx]] - `imports_from` [EXTRACTED]
- [[DataSourcePurposesResult]] - `contains` [EXTRACTED]
- [[DataSourceStatus]] - `contains` [EXTRACTED]
- [[DataSourcesPage.tsx]] - `imports_from` [EXTRACTED]
- [[MAPPING_COMPARISON_POLICY_VALUES]] - `contains` [EXTRACTED]
- [[MappingComparisonPolicy]] - `contains` [EXTRACTED]
- [[MappingWarning]] - `contains` [EXTRACTED]
- [[MappingWarningPurposeSummary]] - `contains` [EXTRACTED]
- [[MappingWarningType]] - `contains` [EXTRACTED]
- [[PublicDataSource]] - `contains` [EXTRACTED]
- [[PublicDataSourceField]] - `contains` [EXTRACTED]
- [[PublicSourceFieldMapping]] - `contains` [EXTRACTED]
- [[ReplaceMappingsResult]] - `contains` [EXTRACTED]
- [[ReplacePurposesResult]] - `contains` [EXTRACTED]
- [[SYNC_FREQUENCY_VALUES]] - `contains` [EXTRACTED]
- [[Step1Connection.test.tsx]] - `imports_from` [EXTRACTED]
- [[Step1Connection.tsx]] - `imports_from` [EXTRACTED]
- [[Step2Schema.tsx]] - `imports_from` [EXTRACTED]
- [[Step3Mapping.test.tsx]] - `imports_from` [EXTRACTED]
- [[Step3Mapping.tsx]] - `imports_from` [EXTRACTED]
- [[Step4Purposes.tsx]] - `imports_from` [EXTRACTED]
- [[Step5Declarations.tsx]] - `imports_from` [EXTRACTED]
- [[SyncFrequency]] - `contains` [EXTRACTED]
- [[SyncHistoryTable.tsx]] - `imports_from` [EXTRACTED]
- [[SyncJob]] - `contains` [EXTRACTED]
- [[SyncStatus]] - `contains` [EXTRACTED]
- [[api-client.ts]] - `imports_from` [EXTRACTED]
- [[cacheMappingsResult()]] - `contains` [EXTRACTED]
- [[cachePurposesResult()]] - `contains` [EXTRACTED]
- [[employeePut()]] - `contains` [EXTRACTED]
- [[employeeTokenStore]] - `imports` [EXTRACTED]
- [[extractMessage()]] - `contains` [EXTRACTED]
- [[mappingsQueryKey()]] - `contains` [EXTRACTED]
- [[purposesQueryKey()]] - `contains` [EXTRACTED]
- [[refreshEmployeeTokenForPut()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/data-sources-apits
---
source_file: "dpdp-platform/backend/src/modules/principals/principals.service.ts"
type: "code"
community: "principals.service.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/principalsservicets
---

# principals.service.ts

## Connections
- [[@nestjscommon_1]] - `imports_from` [EXTRACTED]
- [[@prismaclient_1]] - `imports_from` [EXTRACTED]
- [[AccessLogService]] - `imports` [EXTRACTED]
- [[MaskingService]] - `imports` [EXTRACTED]
- [[PRINCIPALS_PAGE_SIZE]] - `imports` [EXTRACTED]
- [[PRINCIPAL_DETAIL_SELECT]] - `contains` [EXTRACTED]
- [[PrincipalFieldRow]] - `contains` [EXTRACTED]
- [[PrincipalListItem]] - `contains` [EXTRACTED]
- [[PrincipalsService]] - `contains` [EXTRACTED]
- [[PrismaService]] - `imports` [EXTRACTED]
- [[ResolvedPrincipalField]] - `imports` [EXTRACTED]
- [[ScopedTransactionClient]] - `imports` [EXTRACTED]
- [[SearchRow]] - `contains` [EXTRACTED]
- [[TenantContext]] - `imports` [EXTRACTED]
- [[access-log.service.ts]] - `imports_from` [EXTRACTED]
- [[buildPrincipalSearchQuery()]] - `imports` [EXTRACTED]
- [[field-provenance.ts]] - `imports_from` [EXTRACTED]
- [[lineage.service.ts]] - `imports_from` [EXTRACTED]
- [[list-principals.dto.ts]] - `imports_from` [EXTRACTED]
- [[masking.service.ts]] - `imports_from` [EXTRACTED]
- [[me.service.ts]] - `imports_from` [EXTRACTED]
- [[pickDisplayName()]] - `imports` [EXTRACTED]
- [[principal-search-query.ts]] - `imports_from` [EXTRACTED]
- [[principals.controller.ts]] - `imports_from` [EXTRACTED]
- [[principals.module.ts]] - `imports_from` [EXTRACTED]
- [[prisma.service.ts]] - `imports_from` [EXTRACTED]
- [[resolveProvenance()]] - `imports` [EXTRACTED]
- [[scoped-transaction-client.ts]] - `imports_from` [EXTRACTED]
- [[tenant-context.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/principalsservicets
---
source_file: "dpdp-platform/backend/src/modules/organizations/organizations.service.ts"
type: "code"
community: "prisma.service.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/prismaservicets
---

# organizations.service.ts

## Connections
- [[@nestjscommon_1]] - `imports_from` [EXTRACTED]
- [[@prismaclient_1]] - `imports_from` [EXTRACTED]
- [[AuditService]] - `imports` [EXTRACTED]
- [[OrganizationsService]] - `contains` [EXTRACTED]
- [[PrismaService]] - `imports` [EXTRACTED]
- [[SDF_FIELD_NAMES]] - `contains` [EXTRACTED]
- [[THIRD_SCHEDULE_FIELD_NAMES]] - `contains` [EXTRACTED]
- [[TenantContext]] - `imports` [EXTRACTED]
- [[UpdateOrganizationDto]] - `imports` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[organizations.controller.ts]] - `imports_from` [EXTRACTED]
- [[organizations.module.ts]] - `imports_from` [EXTRACTED]
- [[prisma.service.ts]] - `imports_from` [EXTRACTED]
- [[tenant-context.ts]] - `imports_from` [EXTRACTED]
- [[update-organization.dto.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/prismaservicets
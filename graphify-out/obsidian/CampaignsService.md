---
source_file: "dpdp-platform/backend/src/modules/messaging/campaigns/campaigns.service.ts"
type: "code"
community: "CampaignsService"
location: "L160"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/CampaignsService
---

# CampaignsService

## Connections
- [[dot-approve()_1]] - `method` [EXTRACTED]
- [[dot-compileFilter()]] - `method` [EXTRACTED]
- [[dot-constructor()_3]] - `references` [EXTRACTED]
- [[dot-constructor()_102]] - `references` [EXTRACTED]
- [[dot-constructor()_103]] - `references` [EXTRACTED]
- [[dot-constructor()_154]] - `method` [EXTRACTED]
- [[dot-create()_22]] - `method` [EXTRACTED]
- [[dot-deliverRecipient()]] - `method` [EXTRACTED]
- [[dot-extractAndValidateVariables()_1]] - `method` [EXTRACTED]
- [[dot-get()_13]] - `method` [EXTRACTED]
- [[dot-list()_2]] - `method` [EXTRACTED]
- [[dot-listRecipients()_1]] - `method` [EXTRACTED]
- [[dot-maybeFinalizeCampaign()]] - `method` [EXTRACTED]
- [[dot-resolveRecipients()]] - `method` [EXTRACTED]
- [[dot-send()_7]] - `method` [EXTRACTED]
- [[Injectable_63]] - `references` [EXTRACTED]
- [[breach.service.ts]] - `imports` [EXTRACTED]
- [[campaign-send.processor.ts]] - `imports` [EXTRACTED]
- [[campaigns.controller.ts]] - `imports` [EXTRACTED]
- [[campaigns.e2e-spec.ts]] - `imports` [EXTRACTED]
- [[campaigns.module.ts]] - `imports` [EXTRACTED]
- [[campaigns.service.ts]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/CampaignsService
---
source_file: "dpdp-platform/backend/src/modules/children/child-exemptions.service.ts"
type: "code"
community: "child-exemptions.service.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/child-exemptionsservicets
---

# child-exemptions.service.ts

## Connections
- [[@nestjscommon]] - `imports_from` [EXTRACTED]
- [[@prismaclient_1]] - `imports_from` [EXTRACTED]
- [[AccessTokenPayload]] - `imports` [EXTRACTED]
- [[AuditService]] - `imports` [EXTRACTED]
- [[ChildExemptionsService]] - `contains` [EXTRACTED]
- [[CreateExemptionClaimDto]] - `imports` [EXTRACTED]
- [[EXEMPTION_CLAIM_PUBLIC_SELECT]] - `contains` [EXTRACTED]
- [[ExemptionClaimRow]] - `contains` [EXTRACTED]
- [[ListExemptionClaimsDto]] - `imports` [EXTRACTED]
- [[PrismaService]] - `imports` [EXTRACTED]
- [[PublicExemptionClaim]] - `contains` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[child-exemptions.controller.ts]] - `imports_from` [EXTRACTED]
- [[children.module.ts]] - `imports_from` [EXTRACTED]
- [[create-exemption-claim.dto.ts]] - `imports_from` [EXTRACTED]
- [[list-exemption-claims.dto.ts]] - `imports_from` [EXTRACTED]
- [[prisma.service.ts]] - `imports_from` [EXTRACTED]
- [[toPublicExemptionClaim()]] - `contains` [EXTRACTED]
- [[token.service.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/child-exemptionsservicets
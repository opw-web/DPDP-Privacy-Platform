---
source_file: "dpdp-platform/backend/test/step6-conflict-acceptance.e2e-spec.ts"
type: "code"
community: "step6-conflict-acceptance.e2e-spec.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/step6-conflict-acceptancee2e-spects
---

# step6-conflict-acceptance.e2e-spec.ts

## Connections
- [[@nestjscommon_1]] - `imports_from` [EXTRACTED]
- [[@nestjstesting_1]] - `imports_from` [EXTRACTED]
- [[@prismaclient_1]] - `imports_from` [EXTRACTED]
- [[AppModule]] - `imports` [EXTRACTED]
- [[EmployeeSession_4]] - `contains` [EXTRACTED]
- [[FieldMapping_1]] - `contains` [EXTRACTED]
- [[MockHttpServer]] - `imports` [EXTRACTED]
- [[PERMISSIONS]] - `imports` [EXTRACTED]
- [[PrismaService]] - `imports` [EXTRACTED]
- [[SyncPipelineService]] - `imports` [EXTRACTED]
- [[TenantContext]] - `imports` [EXTRACTED]
- [[TenantStore]] - `imports` [EXTRACTED]
- [[app.module.ts]] - `imports_from` [EXTRACTED]
- [[argon2_1]] - `imports_from` [EXTRACTED]
- [[authenticated()_2]] - `contains` [EXTRACTED]
- [[bareDataSource()]] - `contains` [EXTRACTED]
- [[createDataSource()_4]] - `contains` [EXTRACTED]
- [[employeeWithPermissions()_1]] - `contains` [EXTRACTED]
- [[ensurePermission()_13]] - `contains` [EXTRACTED]
- [[fixture_4]] - `contains` [EXTRACTED]
- [[mock-http-server.ts]] - `imports_from` [EXTRACTED]
- [[organization()_4]] - `contains` [EXTRACTED]
- [[pagedHandler()_1]] - `contains` [EXTRACTED]
- [[prisma.service.ts]] - `imports_from` [EXTRACTED]
- [[putLegacyMappings()]] - `contains` [EXTRACTED]
- [[seedpermissions.ts]] - `imports_from` [EXTRACTED]
- [[startServer()_1]] - `contains` [EXTRACTED]
- [[stripComparisonPolicy()]] - `contains` [EXTRACTED]
- [[supertest]] - `imports_from` [EXTRACTED]
- [[sync-pipeline.service.ts]] - `imports_from` [EXTRACTED]
- [[tenant()_5]] - `contains` [EXTRACTED]
- [[tenant-context.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/step6-conflict-acceptancee2e-spects
---
source_file: "dpdp-platform/frontend/src/components/shared/DeadlinePill.tsx"
type: "code"
community: "cn"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/cn
---

# DeadlinePill.tsx

## Connections
- [[AssessmentRow.tsx]] - `imports_from` [EXTRACTED]
- [[BADGE_VARIANT]] - `contains` [EXTRACTED]
- [[Badge()]] - `imports` [EXTRACTED]
- [[BadgeProps]] - `imports` [EXTRACTED]
- [[Band]] - `contains` [EXTRACTED]
- [[DeadlinePill()]] - `contains` [EXTRACTED]
- [[DeadlinePill.test.tsx]] - `imports_from` [EXTRACTED]
- [[DeadlinePillProps]] - `contains` [EXTRACTED]
- [[badge.tsx]] - `imports_from` [EXTRACTED]
- [[bandFor()]] - `contains` [EXTRACTED]
- [[cn()]] - `imports` [EXTRACTED]
- [[format.ts]] - `imports_from` [EXTRACTED]
- [[formatDeadlineText()]] - `imports` [EXTRACTED]
- [[useCountdown()]] - `imports` [EXTRACTED]
- [[useCountdown.ts]] - `imports_from` [EXTRACTED]
- [[utils.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/cn
---
source_file: "dpdp-platform/backend/src/modules/messaging/templates/templates.service.ts"
type: "code"
community: "templates.service.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/templatesservicets
---

# templates.service.ts

## Connections
- [[AuditService]] - `imports` [EXTRACTED]
- [[BREACH_NOTIFICATION_REQUIRED_ELEMENTS]] - `imports` [EXTRACTED]
- [[CreateTemplateDto]] - `imports` [EXTRACTED]
- [[DisallowedTemplateSyntaxError]] - `imports` [EXTRACTED]
- [[MESSAGE_TEMPLATE_PUBLIC_SELECT]] - `contains` [EXTRACTED]
- [[MissingOrganizationContactError]] - `imports` [EXTRACTED]
- [[MissingRequiredVariableError]] - `imports` [EXTRACTED]
- [[PreviewTemplateDto]] - `imports` [EXTRACTED]
- [[PrismaService]] - `imports` [EXTRACTED]
- [[PublicMessageTemplate]] - `contains` [EXTRACTED]
- [[RenderedTemplate]] - `imports` [EXTRACTED]
- [[TemplateRenderError]] - `imports` [EXTRACTED]
- [[TemplateVariableName]] - `imports` [EXTRACTED]
- [[TemplatesService]] - `contains` [EXTRACTED]
- [[UnknownTemplateVariableError]] - `imports` [EXTRACTED]
- [[UpdateTemplateDto]] - `imports` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[campaigns.service.ts]] - `imports_from` [EXTRACTED]
- [[create-template.dto.ts]] - `imports_from` [EXTRACTED]
- [[duplicateCodeMessage()_1]] - `contains` [EXTRACTED]
- [[extractTemplateVariables()]] - `imports` [EXTRACTED]
- [[preview-template.dto.ts]] - `imports_from` [EXTRACTED]
- [[prisma.service.ts]] - `imports_from` [EXTRACTED]
- [[renderOrganizationMessageTemplate()]] - `imports` [EXTRACTED]
- [[template-renderer.ts]] - `imports_from` [EXTRACTED]
- [[templates.controller.ts]] - `imports_from` [EXTRACTED]
- [[templates.module.ts]] - `imports_from` [EXTRACTED]
- [[toBadRequest()]] - `contains` [EXTRACTED]
- [[update-template.dto.ts]] - `imports_from` [EXTRACTED]
- [[whitelisted-variables.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/templatesservicets
---
source_file: "dpdp-platform/frontend/src/fiduciary/pages/RegistersPage.tsx"
type: "code"
community: "PrincipalDetailPage.tsx"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/PrincipalDetailPagetsx
---

# RegistersPage.tsx

## Connections
- [[RecipientsTab()]] - `imports` [EXTRACTED]
- [[RecipientsTab.tsx]] - `imports_from` [EXTRACTED]
- [[RegistersPage()]] - `contains` [EXTRACTED]
- [[RetentionTab()]] - `imports` [EXTRACTED]
- [[RetentionTab.tsx]] - `imports_from` [EXTRACTED]
- [[SecurityMeasuresTab()_1]] - `imports` [EXTRACTED]
- [[SecurityMeasuresTab.tsx]] - `imports_from` [EXTRACTED]
- [[SharingTab()]] - `imports` [EXTRACTED]
- [[SharingTab.tsx]] - `imports_from` [EXTRACTED]
- [[Tabs]] - `imports` [EXTRACTED]
- [[TabsContent]] - `imports` [EXTRACTED]
- [[TabsList]] - `imports` [EXTRACTED]
- [[TabsTrigger]] - `imports` [EXTRACTED]
- [[TransfersTab()]] - `imports` [EXTRACTED]
- [[TransfersTab.tsx]] - `imports_from` [EXTRACTED]
- [[router.tsx]] - `imports_from` [EXTRACTED]
- [[tabs.tsx]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/PrincipalDetailPagetsx
---
source_file: "dpdp-platform/frontend/src/fiduciary/lib/enum-options.ts"
type: "code"
community: "SecurityMeasuresTab.tsx"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/SecurityMeasuresTabtsx
---

# enum-options.ts

## Connections
- [[AuditPage.tsx]] - `imports_from` [EXTRACTED]
- [[BreachWizardPage.tsx]] - `imports_from` [EXTRACTED]
- [[DATA_CATEGORY_OPTIONS]] - `contains` [EXTRACTED]
- [[DATA_CATEGORY_VALUES_1]] - `contains` [EXTRACTED]
- [[EnumOption]] - `contains` [EXTRACTED]
- [[LAWFUL_BASIS_OPTIONS]] - `contains` [EXTRACTED]
- [[LAWFUL_BASIS_VALUES]] - `contains` [EXTRACTED]
- [[LEGITIMATE_USE_LIMB_OPTIONS]] - `contains` [EXTRACTED]
- [[LEGITIMATE_USE_LIMB_VALUES]] - `contains` [EXTRACTED]
- [[MessagingCampaignDetailPage.tsx]] - `imports_from` [EXTRACTED]
- [[PrincipalDetailPage.tsx]] - `imports_from` [EXTRACTED]
- [[PrincipalEvidencePage.tsx]] - `imports_from` [EXTRACTED]
- [[PrincipalsPage.tsx]] - `imports_from` [EXTRACTED]
- [[PurposeForm.tsx]] - `imports_from` [EXTRACTED]
- [[PurposesPage.tsx]] - `imports_from` [EXTRACTED]
- [[RECIPIENT_TYPE_OPTIONS]] - `contains` [EXTRACTED]
- [[RECIPIENT_TYPE_VALUES]] - `contains` [EXTRACTED]
- [[RETENTION_LEGAL_BASIS_TYPE_OPTIONS]] - `contains` [EXTRACTED]
- [[RETENTION_LEGAL_BASIS_TYPE_VALUES]] - `contains` [EXTRACTED]
- [[RETENTION_TRIGGER_TYPE_OPTIONS]] - `contains` [EXTRACTED]
- [[RETENTION_TRIGGER_TYPE_VALUES]] - `contains` [EXTRACTED]
- [[RETENTION_UNIT_OPTIONS]] - `contains` [EXTRACTED]
- [[RETENTION_UNIT_VALUES]] - `contains` [EXTRACTED]
- [[RecipientsTab.tsx]] - `imports_from` [EXTRACTED]
- [[RequestWorkPanel.tsx]] - `imports_from` [EXTRACTED]
- [[RequestsPage.tsx]] - `imports_from` [EXTRACTED]
- [[RetentionTab.tsx]] - `imports_from` [EXTRACTED]
- [[RulePanel.tsx]] - `imports_from` [EXTRACTED]
- [[SECURITY_MEASURE_TYPE_OPTIONS]] - `contains` [EXTRACTED]
- [[SECURITY_MEASURE_TYPE_VALUES]] - `contains` [EXTRACTED]
- [[SECURITY_RULE_REFERENCE_OPTIONS]] - `contains` [EXTRACTED]
- [[SECURITY_RULE_REFERENCE_VALUES]] - `contains` [EXTRACTED]
- [[SdfDeclarationCard.tsx]] - `imports_from` [EXTRACTED]
- [[SecurityMeasuresTab.tsx]] - `imports_from` [EXTRACTED]
- [[SettingsCompliancePage.tsx]] - `imports_from` [EXTRACTED]
- [[SettingsPage.tsx]] - `imports_from` [EXTRACTED]
- [[SharingTab.tsx]] - `imports_from` [EXTRACTED]
- [[Step1Connection.tsx]] - `imports_from` [EXTRACTED]
- [[Step3Mapping.tsx]] - `imports_from` [EXTRACTED]
- [[TransfersTab.tsx]] - `imports_from` [EXTRACTED]
- [[humanizeEnum()]] - `contains` [EXTRACTED]
- [[titleCase()]] - `contains` [EXTRACTED]
- [[toOptions()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/SecurityMeasuresTabtsx
---
type: community
members: 15
---

# scripts

**Members:** 15 nodes

## Members
- [[build]] - code - dpdp-platform/backend/package.json
- [[format]] - code - dpdp-platform/backend/package.json
- [[lint]] - code - dpdp-platform/backend/package.json
- [[scripts]] - code - dpdp-platform/backend/package.json
- [[seed]] - code - dpdp-platform/backend/package.json
- [[seedprincipals]] - code - dpdp-platform/backend/package.json
- [[seedscale]] - code - dpdp-platform/backend/package.json
- [[start]] - code - dpdp-platform/backend/package.json
- [[startdebug]] - code - dpdp-platform/backend/package.json
- [[startdev]] - code - dpdp-platform/backend/package.json
- [[startprod]] - code - dpdp-platform/backend/package.json
- [[test]] - code - dpdp-platform/backend/package.json
- [[testcov]] - code - dpdp-platform/backend/package.json
- [[teste2e]] - code - dpdp-platform/backend/package.json
- [[testwatch]] - code - dpdp-platform/backend/package.json

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/scripts
SORT file.name ASC
```

## Connections to other communities
- 1 edge to [[_COMMUNITY_backendpackage.json]]

## Top bridge nodes
- [[scripts]] - degree 15, connects to 1 community
---
source_file: "dpdp-platform/frontend/src/fiduciary/pages/MessagingCampaignDetailPage.tsx"
type: "code"
community: "cn"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/cn
---

# MessagingCampaignDetailPage.tsx

## Connections
- [[Badge()]] - `imports` [EXTRACTED]
- [[Button]] - `imports` [EXTRACTED]
- [[Campaign_1]] - `contains` [EXTRACTED]
- [[CampaignRecipient]] - `contains` [EXTRACTED]
- [[Card]] - `imports` [EXTRACTED]
- [[CardContent]] - `imports` [EXTRACTED]
- [[CardHeader]] - `imports` [EXTRACTED]
- [[CardTitle]] - `imports` [EXTRACTED]
- [[DateTime()]] - `imports` [EXTRACTED]
- [[DateTime.tsx]] - `imports_from` [EXTRACTED]
- [[MessagingCampaignDetailPage()]] - `contains` [EXTRACTED]
- [[MessagingCampaignDetailPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[RecipientStatusCounts]] - `contains` [EXTRACTED]
- [[Skeleton()]] - `imports` [EXTRACTED]
- [[Skeleton.tsx]] - `imports_from` [EXTRACTED]
- [[Table]] - `imports` [EXTRACTED]
- [[TableBody]] - `imports` [EXTRACTED]
- [[TableCell]] - `imports` [EXTRACTED]
- [[TableHead]] - `imports` [EXTRACTED]
- [[TableHeader]] - `imports` [EXTRACTED]
- [[TableRow]] - `imports` [EXTRACTED]
- [[api-client.ts]] - `imports_from` [EXTRACTED]
- [[badge.tsx]] - `imports_from` [EXTRACTED]
- [[button.tsx]] - `imports_from` [EXTRACTED]
- [[campaignNoticeLinkText()]] - `contains` [EXTRACTED]
- [[canApproveCampaign()]] - `contains` [EXTRACTED]
- [[canSendCampaign()]] - `contains` [EXTRACTED]
- [[card.tsx]] - `imports_from` [EXTRACTED]
- [[deliveredToNobody()]] - `contains` [EXTRACTED]
- [[employeeApiClient]] - `imports` [EXTRACTED]
- [[enum-options.ts]] - `imports_from` [EXTRACTED]
- [[humanizeEnum()]] - `imports` [EXTRACTED]
- [[libauth.ts]] - `imports_from` [EXTRACTED]
- [[recipientReasonText()]] - `contains` [EXTRACTED]
- [[router.tsx]] - `imports_from` [EXTRACTED]
- [[shouldShowCampaignApproval()]] - `contains` [EXTRACTED]
- [[statusBadgeVariant()]] - `contains` [EXTRACTED]
- [[summarizeRecipientStatuses()]] - `contains` [EXTRACTED]
- [[table.tsx]] - `imports_from` [EXTRACTED]
- [[useEmployeeAuth()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/cn
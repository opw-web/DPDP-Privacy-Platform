---
source_file: "dpdp-platform/backend/src/modules/retention/pre-erasure-notice.service.ts"
type: "code"
community: "prisma.service.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/prismaservicets
---

# pre-erasure-notice.service.ts

## Connections
- [[@nestjscommon_1]] - `imports_from` [EXTRACTED]
- [[AuditService]] - `imports` [EXTRACTED]
- [[NotificationSendInput]] - `imports` [EXTRACTED]
- [[NotificationsService]] - `imports` [EXTRACTED]
- [[PreErasureNoticeService]] - `contains` [EXTRACTED]
- [[PreErasureNoticeSummary]] - `contains` [EXTRACTED]
- [[PrismaService]] - `imports` [EXTRACTED]
- [[TenantContext]] - `imports` [EXTRACTED]
- [[TenantStore]] - `imports` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[buildCancellationReason()]] - `contains` [EXTRACTED]
- [[buildPreErasureNoticeBody()]] - `contains` [EXTRACTED]
- [[lockRetentionWorkflow()]] - `imports` [EXTRACTED]
- [[notification-provider.interface.ts]] - `imports_from` [EXTRACTED]
- [[notifications.service.ts]] - `imports_from` [EXTRACTED]
- [[pre-erasure-notice.processor.ts]] - `imports_from` [EXTRACTED]
- [[prisma.service.ts]] - `imports_from` [EXTRACTED]
- [[retention-transaction-lock.util.ts]] - `imports_from` [EXTRACTED]
- [[retention.e2e-spec.ts]] - `imports_from` [EXTRACTED]
- [[retention.module.ts]] - `imports_from` [EXTRACTED]
- [[tenant-context.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/prismaservicets
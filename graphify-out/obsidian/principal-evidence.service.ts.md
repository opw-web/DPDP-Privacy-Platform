---
source_file: "dpdp-platform/backend/src/modules/evidence/principal-evidence.service.ts"
type: "code"
community: "principals.service.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/principalsservicets
---

# principal-evidence.service.ts

## Connections
- [[@nestjscommon]] - `imports_from` [EXTRACTED]
- [[@prismaclient]] - `imports_from` [EXTRACTED]
- [[AccessLogService]] - `imports` [EXTRACTED]
- [[AuditService]] - `imports` [EXTRACTED]
- [[EvidenceBreachInclusion]] - `contains` [EXTRACTED]
- [[EvidenceConsentEvent]] - `contains` [EXTRACTED]
- [[EvidenceMessageReceived]] - `contains` [EXTRACTED]
- [[EvidenceNoticeVersionShown]] - `contains` [EXTRACTED]
- [[EvidenceRequest]] - `contains` [EXTRACTED]
- [[EvidenceRequestEvent]] - `contains` [EXTRACTED]
- [[PrincipalEvidenceFile]] - `contains` [EXTRACTED]
- [[PrincipalEvidenceService]] - `contains` [EXTRACTED]
- [[PrismaService]] - `imports` [EXTRACTED]
- [[PublicInformationRequest_1]] - `imports` [EXTRACTED]
- [[access-log.service.ts]] - `imports_from` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[evidence.e2e-spec.ts]] - `imports_from` [EXTRACTED]
- [[evidence.module.ts]] - `imports_from` [EXTRACTED]
- [[evidencenon-disclosure.ts]] - `imports_from` [EXTRACTED]
- [[principal-evidence-render.ts]] - `imports_from` [EXTRACTED]
- [[principal-evidence.controller.ts]] - `imports_from` [EXTRACTED]
- [[prisma.service.ts]] - `imports_from` [EXTRACTED]
- [[splitNonDisclosureRequests()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/principalsservicets
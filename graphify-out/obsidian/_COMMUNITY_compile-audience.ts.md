---
type: community
members: 40
---

# compile-audience.ts

**Members:** 40 nodes

## Members
- [[dot-constructor()_80]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience-filter.error.ts
- [[AUDIENCE_FILTER_FIELDS]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience-filter.types.ts
- [[AUDIENCE_FILTER_OPERATORS]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience-filter.types.ts
- [[AudienceFilter]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience-filter.types.ts
- [[AudienceFilterError]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience-filter.error.ts
- [[AudienceFilterField]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience-filter.types.ts
- [[AudienceFilterGroup]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience-filter.types.ts
- [[AudienceFilterNode]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience-filter.types.ts
- [[AudienceFilterOperator]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience-filter.types.ts
- [[AudienceFilterRule]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience-filter.types.ts
- [[AudiencePreviewResult]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience.service.ts
- [[CHILD_LIKE_AGE_STATUSES_1]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience.service.ts
- [[audience-filter.error.ts]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience-filter.error.ts
- [[audience-filter.types.ts]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience-filter.types.ts
- [[audience.service.ts]] - code - dpdp-platform/backend/src/modules/messaging/audience/audience.service.ts
- [[compile-audience.spec.ts]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.spec.ts
- [[compile-audience.ts]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[compileAgeStatus()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[compileAudience()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[compileBreachAffected()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[compileCanonicalFieldStringValue()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[compileConsent()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[compileDataSource()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[compileErasureState()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[compileGroup()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[compileHasEmail()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[compileHasField()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[compileLastContactAt()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[compileNode()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[compileRequestStatus()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[compileRule()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[dataSourceMatcher()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[expectBoolean()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[expectDate()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[expectEnumArray()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[expectEnumValue()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[expectString()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[expectStringArray()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[isPlainObject()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts
- [[unsupportedOperator()]] - code - dpdp-platform/backend/src/modules/messaging/audience/compile-audience.ts

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/compile-audiencets
SORT file.name ASC
```

## Connections to other communities
- 6 edges to [[_COMMUNITY_campaigns.service.ts]]
- 5 edges to [[_COMMUNITY_@nestjscommon]]
- 2 edges to [[_COMMUNITY_AudienceService]]
- 2 edges to [[_COMMUNITY_PrismaService]]
- 2 edges to [[_COMMUNITY_@nestjsswagger]]
- 2 edges to [[_COMMUNITY_@prismaclient]]
- 1 edge to [[_COMMUNITY_MaskingService]]
- 1 edge to [[_COMMUNITY_PreviewAudienceDto]]
- 1 edge to [[_COMMUNITY_BreachesController]]
- 1 edge to [[_COMMUNITY_CampaignsService]]
- 1 edge to [[_COMMUNITY_app.module.ts]]
- 1 edge to [[_COMMUNITY_principals.service.ts]]

## Top bridge nodes
- [[audience.service.ts]] - degree 19, connects to 9 communities
- [[compileAudience()]] - degree 9, connects to 5 communities
- [[compile-audience.ts]] - degree 36, connects to 3 communities
- [[AudienceFilterError]] - degree 18, connects to 2 communities
- [[audience-filter.error.ts]] - degree 6, connects to 2 communities
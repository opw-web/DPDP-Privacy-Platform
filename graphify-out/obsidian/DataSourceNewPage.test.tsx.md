---
source_file: "dpdp-platform/frontend/src/fiduciary/pages/DataSourceNewPage.test.tsx"
type: "code"
community: "@tanstack/react-query"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/tanstack/react-query
---

# DataSourceNewPage.test.tsx

## Connections
- [[@tanstackreact-query_1]] - `imports_from` [EXTRACTED]
- [[@testing-libraryreact_1]] - `imports_from` [EXTRACTED]
- [[@testing-libraryuser-event_1]] - `imports_from` [EXTRACTED]
- [[ATTACHED_PURPOSE]] - `contains` [EXTRACTED]
- [[CREATED_DATA_SOURCE]] - `contains` [EXTRACTED]
- [[DataSourceNewPage()]] - `imports` [EXTRACTED]
- [[DataSourceNewPage.tsx]] - `imports_from` [EXTRACTED]
- [[DetailStub()]] - `contains` [EXTRACTED]
- [[employeeLogin()]] - `imports` [EXTRACTED]
- [[employeeLogout()]] - `imports` [EXTRACTED]
- [[jsonResponse()_10]] - `contains` [EXTRACTED]
- [[libauth.ts]] - `imports_from` [EXTRACTED]
- [[loginAndRenderWizard()]] - `contains` [EXTRACTED]
- [[react-router-dom_1]] - `imports_from` [EXTRACTED]
- [[renderWizard()]] - `contains` [EXTRACTED]
- [[vitest_1]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/tanstack/react-query
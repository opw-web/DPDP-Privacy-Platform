---
source_file: "demo-company-server/src/seed/personas.ts"
type: "code"
community: "generate.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/generatets
---

# personas.ts

## Connections
- [[AnyRecord]] - `contains` [EXTRACTED]
- [[DOB_REFERENCE_DATE]] - `contains` [EXTRACTED]
- [[EcommerceRecord]] - `contains` [EXTRACTED]
- [[MarketingRecord]] - `contains` [EXTRACTED]
- [[PersonaPerson]] - `contains` [EXTRACTED]
- [[RESERVED_NAMES]] - `contains` [EXTRACTED]
- [[RESERVED_PINCODES]] - `contains` [EXTRACTED]
- [[SalesRecord]] - `contains` [EXTRACTED]
- [[SupportRecord]] - `contains` [EXTRACTED]
- [[dataset.test.ts]] - `imports_from` [EXTRACTED]
- [[generate.ts]] - `imports_from` [EXTRACTED]
- [[personas]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/generatets
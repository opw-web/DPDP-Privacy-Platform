---
source_file: "dpdp-platform/backend/src/modules/principal-portal/me-rights.service.ts"
type: "code"
community: "@nestjs/common"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/nestjs/common
---

# me-rights.service.ts

## Connections
- [[@nestjscommon]] - `imports_from` [EXTRACTED]
- [[@prismaclient_1]] - `imports_from` [EXTRACTED]
- [[AuditService]] - `imports` [EXTRACTED]
- [[CreateMeRequestDto]] - `imports` [EXTRACTED]
- [[ME_REQUEST_EVENT_SELECT]] - `contains` [EXTRACTED]
- [[MeRightsService]] - `contains` [EXTRACTED]
- [[NOMINATION_PUBLIC_SELECT]] - `contains` [EXTRACTED]
- [[NotificationCallerActor]] - `imports` [EXTRACTED]
- [[NotificationsService]] - `imports` [EXTRACTED]
- [[PrincipalNoticeSummary]] - `contains` [EXTRACTED]
- [[PrincipalNoticeView]] - `contains` [EXTRACTED]
- [[PrismaService]] - `imports` [EXTRACTED]
- [[REQUEST_EVENT_PUBLIC_SELECT]] - `imports` [EXTRACTED]
- [[REQUEST_PUBLIC_SELECT]] - `imports` [EXTRACTED]
- [[RequestsService]] - `imports` [EXTRACTED]
- [[TenantContext]] - `imports` [EXTRACTED]
- [[UpdateMeNominationDto]] - `imports` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[create-me-request.dto.ts]] - `imports_from` [EXTRACTED]
- [[isNoticeLanguageCode()]] - `imports` [EXTRACTED]
- [[jwt-any-actor.guard.ts]] - `imports_from` [EXTRACTED]
- [[languages.ts]] - `imports_from` [EXTRACTED]
- [[me-rights.controller.ts]] - `imports_from` [EXTRACTED]
- [[notifications.service.ts]] - `imports_from` [EXTRACTED]
- [[principal-portal.module.ts]] - `imports_from` [EXTRACTED]
- [[prisma.service.ts]] - `imports_from` [EXTRACTED]
- [[requests.service.ts]] - `imports_from` [EXTRACTED]
- [[tenant-context.ts]] - `imports_from` [EXTRACTED]
- [[update-me-nomination.dto.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/nestjs/common
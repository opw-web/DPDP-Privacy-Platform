---
source_file: "dpdp-platform/frontend/src/lib/auth.ts"
type: "code"
community: "router.tsx"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/routertsx
---

# lib/auth.ts

## Connections
- [[API_BASE]] - `imports` [EXTRACTED]
- [[AppShell.tsx]] - `imports_from` [EXTRACTED]
- [[AuditPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[AuthStatus]] - `contains` [EXTRACTED]
- [[DashboardPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[DataSourceDetailPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[DataSourceNewPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[EmployeeSession_5]] - `contains` [EXTRACTED]
- [[EmployeeSummary]] - `contains` [EXTRACTED]
- [[EmployeesPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[InformationRequestsPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[LinkedRecordsPanel.test.tsx]] - `imports_from` [EXTRACTED]
- [[Listener]] - `contains` [EXTRACTED]
- [[LoginPage.tsx]] - `imports_from` [EXTRACTED]
- [[MessagingCampaignDetailPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[MessagingCampaignDetailPage.tsx]] - `imports_from` [EXTRACTED]
- [[NoticeBuilderPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[PermissionGate.test.tsx]] - `dynamic_import` [EXTRACTED]
- [[PortalShell.tsx]] - `imports_from` [EXTRACTED]
- [[PrincipalDetailPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[PrincipalEvidencePage.test.tsx]] - `imports_from` [EXTRACTED]
- [[PrincipalLoginPage.tsx]] - `imports_from` [EXTRACTED]
- [[PrincipalSession_1]] - `contains` [EXTRACTED]
- [[PrincipalsPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[PurposesPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[RawEmployeeMe]] - `contains` [EXTRACTED]
- [[RecipientsTab.test.tsx]] - `imports_from` [EXTRACTED]
- [[RequestDetailPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[ReviewQueuePage.test.tsx]] - `imports_from` [EXTRACTED]
- [[SdfPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[SettingsCompliancePage.test.tsx]] - `imports_from` [EXTRACTED]
- [[SettingsPage.test.tsx]] - `imports_from` [EXTRACTED]
- [[TransfersTab.tsx]] - `imports_from` [EXTRACTED]
- [[api-client.test.ts]] - `imports_from` [EXTRACTED]
- [[api-client.ts]] - `imports_from` [EXTRACTED]
- [[bootstrapEmployeeSession()]] - `contains` [EXTRACTED]
- [[bootstrapPrincipalSession()]] - `contains` [EXTRACTED]
- [[createAuthStore()]] - `contains` [EXTRACTED]
- [[employeeApiClient]] - `imports` [EXTRACTED]
- [[employeeAuthStore]] - `contains` [EXTRACTED]
- [[employeeLogin()]] - `contains` [EXTRACTED]
- [[employeeLogout()]] - `contains` [EXTRACTED]
- [[employeeTokenStore]] - `imports` [EXTRACTED]
- [[fetchEmployeeMe()]] - `contains` [EXTRACTED]
- [[libpermissions.ts]] - `imports_from` [EXTRACTED]
- [[loginAs()]] - `imports_from` [EXTRACTED]
- [[principalApiClient]] - `imports` [EXTRACTED]
- [[principalAuthStore]] - `contains` [EXTRACTED]
- [[principalLogin()]] - `contains` [EXTRACTED]
- [[principalLogout()]] - `contains` [EXTRACTED]
- [[principalTokenStore]] - `imports` [EXTRACTED]
- [[refreshEmployeeToken()]] - `contains` [EXTRACTED]
- [[refreshPrincipalToken()]] - `contains` [EXTRACTED]
- [[router.test.tsx]] - `imports_from` [EXTRACTED]
- [[router.tsx]] - `imports_from` [EXTRACTED]
- [[toEmployeeSession()]] - `contains` [EXTRACTED]
- [[useEmployeeAuth()]] - `contains` [EXTRACTED]
- [[usePrincipalAuth()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/routertsx
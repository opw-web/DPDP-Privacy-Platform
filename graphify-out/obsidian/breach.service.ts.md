---
source_file: "dpdp-platform/backend/src/modules/breaches/breach.service.ts"
type: "code"
community: "breach.service.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/breachservicets
---

# breach.service.ts

## Connections
- [[@nestjscommon_1]] - `imports_from` [EXTRACTED]
- [[@prismaclient_1]] - `imports_from` [EXTRACTED]
- [[AccessTokenPayload]] - `imports` [EXTRACTED]
- [[AffectedPreview]] - `imports` [EXTRACTED]
- [[AffectedPrincipalsDto]] - `imports` [EXTRACTED]
- [[AudienceFilterError]] - `imports` [EXTRACTED]
- [[AuditService]] - `imports` [EXTRACTED]
- [[BREACH_PUBLIC_SELECT]] - `contains` [EXTRACTED]
- [[BREACH_RULES]] - `contains` [EXTRACTED]
- [[BoardBreachReport]] - `contains` [EXTRACTED]
- [[BreachPrincipalNoticeDispatchQueueService]] - `imports` [EXTRACTED]
- [[BreachService]] - `contains` [EXTRACTED]
- [[CampaignsService]] - `imports` [EXTRACTED]
- [[CompleteObligationDto]] - `imports` [EXTRACTED]
- [[ComplianceService]] - `imports` [EXTRACTED]
- [[CreateBreachDto]] - `imports` [EXTRACTED]
- [[ExtensionDto]] - `imports` [EXTRACTED]
- [[NotificationsService]] - `imports` [EXTRACTED]
- [[PrismaService]] - `imports` [EXTRACTED]
- [[PublicBreach]] - `imports` [EXTRACTED]
- [[ReferenceService]] - `imports` [EXTRACTED]
- [[TRANSITIONS_1]] - `contains` [EXTRACTED]
- [[UpdateBreachDto]] - `imports` [EXTRACTED]
- [[affected-principals.dto.ts]] - `imports_from` [EXTRACTED]
- [[asDate()]] - `contains` [EXTRACTED]
- [[audience-filter.error.ts]] - `imports_from` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[breach-clock.processor.ts]] - `imports_from` [EXTRACTED]
- [[breach-principal-notice-dispatch.processor.ts]] - `imports_from` [EXTRACTED]
- [[breach-principal-notice-dispatch.queue.ts]] - `imports_from` [EXTRACTED]
- [[breach-render.ts]] - `imports_from` [EXTRACTED]
- [[breach.types.ts]] - `imports_from` [EXTRACTED]
- [[breaches.controller.ts]] - `imports_from` [EXTRACTED]
- [[breaches.e2e-spec.ts]] - `imports_from` [EXTRACTED]
- [[breaches.module.ts]] - `imports_from` [EXTRACTED]
- [[campaigns.service.ts]] - `imports_from` [EXTRACTED]
- [[compile-audience.ts]] - `imports_from` [EXTRACTED]
- [[compileAudience()]] - `imports` [EXTRACTED]
- [[complete-obligation.dto.ts]] - `imports_from` [EXTRACTED]
- [[compliance.service.ts]] - `imports_from` [EXTRACTED]
- [[create-breach.dto.ts]] - `imports_from` [EXTRACTED]
- [[extension.dto.ts]] - `imports_from` [EXTRACTED]
- [[notifications.service.ts]] - `imports_from` [EXTRACTED]
- [[parseIds()]] - `contains` [EXTRACTED]
- [[prisma.service.ts]] - `imports_from` [EXTRACTED]
- [[reference.service.ts]] - `imports_from` [EXTRACTED]
- [[token.service.ts]] - `imports_from` [EXTRACTED]
- [[update-breach.dto.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/breachservicets
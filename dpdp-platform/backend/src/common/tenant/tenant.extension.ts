 ...
 */
export function primaryKeyWhereFor(
  modelName: string,
  ...
  if (fields.length === 0) {
    throw new Error(
      `tenant.extension.ts: primaryKeyWhereFor() found no primary-key ` +
  ...
  for (const field of fields) {
    ...
    if (value === undefined) {
      throw new Error(
        `tenant.extension.ts: primaryKeyWhereFor() got undefined for ` +
...
async function runScopedOperation(
  ...
  switch (operation) {
    ...
    case "createMany": {
      ...
      if (scope.kind === "indirect") {
        if ((args as Record<string, unknown>)[FK_ALREADY_VERIFIED_MARKER]) {
          ...
          const { [FK_ALREADY_VERIFIED_MARKER]: _marker, ...rest } =
